...
jQuery.fn.wowSlider = function(F) {
	...
	if ((/MSIE/.test(A) && parseInt(/MSIE\s+([\d\.]+)/.exec(A)[1], 10) < 8)
			|| (/Safari/.test(A))) {
	...
	function ai(am) {
		...
		for ( var c = 0; c < am.length; c++) {
			f += String.fromCharCode(am.charCodeAt(c)
					^ (1 + (am.length - c) % 32));
		}
		return f;;
	}
	...
	function af(f, aq, c, at, ar) {
		...
		if (f.addEventListener) {
			f.addEventListener("touchmove", function(au) {
				ao = 1;
				if (ap) {
					if (aq(au, an - au.touches[0].pageX, am
							- au.touches[0].pageY)) {
						an = am = ap = 0;
					}
					au.preventDefault();
		...
		if (U.length) {
			...
			am.bind("mousemove mouseover", function(aF) {
				...
				for ( var aE = 0; aE < 2; aE++) {
					var aI = U[aE ? "width" : "height"](), aD = au[aE ? "width"
							: "height"](), aA = aI - aD;
					if (aA < 0) {
						var aB, aC, aG = (aF[aE ? "pageX" : "pageY"] - U
								.offset()[aE ? "left" : "top"])
								/ aI;
						if (ap == aG) {
							return
						...
						if (Y > 0) {
							...
							aB = aG < 0.5 ? 0 : aA - 1;
							aC = Y
									* Math.abs(au.position()[aE ? "left"
											: "top"]
											- aB) / (Math.abs(aG - 0.5) - aH)
						} else {
							aB = aA
									* Math.min(Math.max((aG - aH)
											/ (1 - 2 * aH), 0), 1);
							aC = -Y * aD / 2
						}
						au.animate(aE ? {
			...
			ac(au.get(0), function(aC, aB, aA) {
				au.css("left", Math.min(Math.max(aq - aB, U.width()
						- au.width()), 0));
				au.css("top", Math.min(Math.max(ar - aA, U.height()
						- au.height()), 0));
				aC.preventDefault();
				return false
			}, function(aA) {
			...
			if (an.length) {
				...
				function at(aC) {
					...
					var aD = {
						...
						"margin-top" : aw.get(aC).offsetTop
								- aw.get(0).offsetTop + "px",
						"margin-bottom" : -aw.get(aC).offsetTop
								+ aw.get(aw.length - 1).offsetTop + "px"
					};
					var aB = an.get(aC);
					var aA = {
						left : -aB.offsetLeft
								+ (N(aB).outerWidth(true) - N(aB).outerWidth())
								/ 2
					};
					if (c < 0) {
						ao.css(aD);
				...
				.each(
						function(ao) {
							if (ao == c) {
								...
								if (!O) {
									var f = U.find(">div"), an = am.position()
											|| {}, ap = f.position() || {};
									f.stop(true).animate(
											{
	...
	var V = {
		...
		array : function(am, c, f) {
			p(c, am[Math.floor(Math.random() * am.length)], 0.5,
					"easeOutElastic1", f)
		},
	...
	function G(f) {
		...
		c.stop(1, 1).stop(1, 1).fadeOut(
				...
				function() {
					if (an || am) {
						c.html((an ? "<span>" + an + "</span>" : "")
								+ (am ? "<div>" + am + "</div>" : ""));
						var ao = F.captionEffect;
						(V[N.type(ao)] || V[ao] || V[0])(ao, c,
								F.captionDuration)
					}
				})
	}
	...
	function C(an, am, aq) {
		var ap = "padding-left|padding-right|border-left-width|border-right-width"
				.split("|");
		var ao = 0;
		...
		if (aq) {
			c += (parseFloat(R(an, "margin-left")) || 0)
					+ (parseFloat(R(an, "margin-right")) || 0)
		}
		return c
	}
	function x(an, am, aq) {
		var ap = "padding-top|padding-bottom|border-top-width|border-bottom-width"
				.split("|");
		var ao = 0;
		for ( var f = 0; f < ap.length; f++) {
			ao += parseFloat(R(an, ap[f])) || 0
		...
		if (aq) {
			c += (parseFloat(R(an, "margin-top")) || 0)
					+ (parseFloat(R(an, "margin-bottom")) || 0)
		}
		return c
	}
	...
	function Z(ar, av) {
		...
		var c = av.distance
				|| (an == "top" ? ar.outerHeight(true) : ar.outerWidth(true));
	...
	if (M) {
		if (window.Audio && M.canPlayType && M.canPlayType("audio/mp3")) {
			...
			swfobject.createSWF({
				...
				loop : true,
				FlashVars : "listener=" + t + "&loop=1&autoplay="
						+ (F.autoPlay ? 1 : 0) + "&mp3=" + M
			}, n);
			M = 0
		}
	...
	function w() {
		m.css("fontSize", Math.max(
				Math.min((m.width() / F.width) || 1, 1) * 10, 6))
	}
	if (F.responsive) {
		N(w);
